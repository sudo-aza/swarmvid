...
| 43 | **Run TTS batch for all 28 scenes.** Use `generate_tts_batch.py --resume` (subprocess-per-chunk for memory isolation). 175 segments, ~1852 chunks at 150 chars each. Estimated ~14 hours CPU. After generation: update scene JSON `duration_s` from actual audio, re-render all 28 MP4s via `render_scene_v2.py`. | Programmer | **pending** | 2026-06-21 |
