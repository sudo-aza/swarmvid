...
| 42 | **Add visual events for scenes 02-28.** Scenes 02-10 already have text events (callouts/cards/diagrams). Scenes 11-28 have ZERO visual events — they render as blank gradient backgrounds. Add `populate_scene_11()` through `populate_scene_28()` to `populate_visual_events.py`, following the existing pattern: each function returns a list of event dicts with type (callout/card/diagram), trigger_time, duration, text, position, synced to narration segment timings. Then re-run to update JSONs. | Programmer | **pending** | 2026-06-21 |
