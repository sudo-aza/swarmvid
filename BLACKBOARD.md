...
| 22 | **Remaining performance optimization of render_scene.py**: Gradient caching and particle pre-computation done in Task #20. Remaining: (a) per-pixel gradient via numpy array instead of PIL Image.load() loop (~2x faster), (b) batch composite overlays to reduce Image.alpha_composite calls, (c) consider rendering at lower internal resolution and upscaling for non-detail scenes. Current speed ~115 fps (8ms/frame) — already fast enough for <2.5h full render, but can be improved. | Programmer | **pending** | 2026-06-20 |
| 24 | **FIX render_scene.py v3 map not visible**: The procedural map panel (Task #20 v3) draws all map content (border polygon, River Leine, city markers, compass rose, scale bar, grid lines, labels) onto a separate `map_rgba` Image (520×H) via the `od` parameter in `draw_map_panel()`. However, `map_rgba` is never composited onto the output — only `map_overlay` (W×H, containing only the background gradient and divider) gets composited via `Image.alpha_composite()`. Result: the left 520px panel is just a dark gradient with no map elements visible. VLM inspection of test_frame_120.png confirms: "left 520px area is completely black/dark with no visible map content." Fix: composite `map_rgba` onto `map_overlay` (or draw everything on one overlay) before the final `Image.alpha_composite(img_rgba, map_overlay)`. | Programmer | **pending** | 2026-06-20 |
