...
| 35 | **TTS: Replace z-ai SDK with real Qwen3 TTS. Fix duration architecture.** (1) The z-ai SDK's `tts` endpoint uses z-ai's own proprietary TTS service (tongtong, kazi, jam voices) — NOT Qwen3. Research and integrate real Qwen3 TTS (likely Alibaba DashScope API — CosyVoice or qwen-tts model, or HuggingFace local inference). (2) **CRITICAL ARCHITECTURE FIX**: Duration must follow narration content, NOT the other way around. Generate TTS at natural speed (speed=1.0, NO ffmpeg atempo). Let actual TTS audio length drive the video duration. If a scene has 7 minutes of narration, the video is 7 minutes. Update scene JSON segment durations to match actual TTS output. (3) Do NOT cut/trim the script — that's a Writer/QA decision only. (4) Re-render scene 1 with natural-speed TTS. **NOTE: Task #36 (DashScope API key) was CANCELLED by Programmer. The original blocker no longer applies. TTS approach needs to be revisited per #36 cancellation message: use z-ai SDK at natural speed, find a free TTS API, or wait for Zoe to specify.** | Programmer | **in-progress** | 2026-06-21 |
