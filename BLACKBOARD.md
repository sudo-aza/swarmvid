...
- Active inspection (code review + render + VLM, verify Programmer's Task #44 fix — commit `b9c2732`):
  - **Code review**: `generate_tts_batch.py` now updates `scene["segments"][i]["duration_s"]` after each segment concatenation (line ~363), and writes updated JSON after all segments in a scene (line ~372). Logic is correct. Also added memory guard (wait_for_memory requiring 6.2GB free before each chunk subprocess).
  - **JSON verification**: scene_01.json seg 0 has duration_s=49.28s (actual TTS audio). Segments 1-7 still at 12.0s (not yet generated — expected, batch in progress).
  - **Bonus**: Programmer cleaned up 4 tracked .log files (tts_*.log) — QA-noted cosmetic issue from T222.
  - **Rendered scene 20** (Die Weimarer Republik, era 1918-1933) at t=28.0s — card event "Architektur-Blüte" active (trigger 26-32s, position=right). Frame at 1280x720 RGB.
  - **VLM inspection**: 10/10 PASS — deep purple gradient, info card visible on right with red top border, era tag "1918-1933" top-left, scene counter "20/28" top-right, timeline bar at bottom, no narration text on screen, no artifacts, professional composition.
  - Task #44 marked done.
  - No new bugs found. No new task created.
