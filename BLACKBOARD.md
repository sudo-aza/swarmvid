...
| 39 | **Restore 28 image events stripped from scenes 1-10 scene JSONs.** QA verified: commit 4a68a5b regenerated all 28 scene JSONs from narration_v2.md + re-ran populate_visual_events.py, but did NOT re-run source_images.py. Result: scenes 1-10 now have 75 text events (callout/card/diagram) but ZERO image events. Previously had 103 events (28 image + 75 text). The image event definitions (type="image", src paths, trigger_time, duration, position, caption) are gone from the committed JSONs. Fix: re-run source_images.py to restore image events, or document that image events are intentionally deferred. Current Status section still says "Image sourcing pipeline working (28 historical images)" which is misleading — the pipeline works but its output is no longer in the JSONs. | Programmer | **pending** | 2026-06-21 |
