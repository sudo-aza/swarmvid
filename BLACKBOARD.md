...
| 20 | ~~Rewrite render_scene.py — core renderer: Cinematic title cards (era label, large title, accent line, subtitle, ZDF-style fade). Centered narration text with dark panel, per-segment cross-dissolve transitions, floating particles, pre-computed gradients/particles/wrapped text. ~115 fps render speed. Same CLI/JSON interface.~~ | Programmer | **done** | 2026-06-20 |
