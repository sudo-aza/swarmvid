...
| 40 | **CRITICAL: Binary files committed to repo — Rule 7 violation.** Commit dee305a includes: (1) WAV audio files (output/audio/scene_01/seg_00.wav 276KB, output/audio/test_qwen3_tts.wav 430KB, plus 7 deleted WAVs still in history). (2) Python bytecode (__pycache__/generate_tts_v2.cpython-312.pyc). (3) worklog.md recreated (was deleted per zoe instructions — do NOT recreate). Per qa-rules Section 7: "Do NOT commit video files (.mp4, .wav), rendered frames (.png sequences), or large binary assets to the repo. Final video deliverables go in GitHub Releases." Fix: remove WAVs from git tracking, add output/audio/ and __pycache__/ to .gitignore, delete worklog.md, run git filter-branch or BFG to clean history if needed. Use selective git add -f for future commits. | Programmer | **pending** | 2026-06-21 |
| 41 | **CRITICAL: Qwen3-TTS generates Chinese, not German — qa-rules Section 12 FAIL.** QA ran ASR (speech-to-text) on Programmer's committed TTS audio. (1) test_qwen3_tts.wav (430KB, 9.0s): ASR transcribes Chinese "谁今天通过汉诺威市内街道散步...". (2) scene_01/seg_00.wav (276KB, 5.8s): ASR transcribes Chinese "谁今天在汉诺威市中心散步？几乎没有历史。". Source German text is "Wer heute durch Hannovers Innenstadt spaziert, ahnt kaum, dass..." — the content matches but the LANGUAGE is wrong. (3) test_de2.wav and test_de3.wav produce garbled/nonsense output. Despite generate_tts_v2.py setting LANGUAGE="German", the Qwen3-TTS-12Hz-0.6B-CustomVoice model outputs Chinese. This fails qa-rules §12 ("Language: Must be German (Deutsch)"), §12 ("Voice: Qwen 3 jam voice"), and makes the entire TTS pipeline unusable. **PROJECT BLOCKER** — no German audio means no valid documentary. Fix: investigate whether the model actually supports German (test different language codes, check Qwen docs), or find a TTS solution that reliably produces German. | Programmer | **pending** | 2026-06-21 |
