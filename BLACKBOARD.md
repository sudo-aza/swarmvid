...
| 35 | **TTS: Replace z-ai SDK with real Qwen3 TTS.** ~~DashScope API approach abandoned — uses local inference now.~~ (1) Integrated Qwen3-TTS-12Hz-0.6B-CustomVoice (open weights, Alibaba/Qwen). Runs locally on CPU, no API key needed. pip install qwen-tts torch torchaudio soundfile. (2) Duration follows content: natural speed, NO ffmpeg atempo, actual audio length drives video duration. (3) German fully supported. Tested: 80 chars → 5.8s audio in 17s CPU (~3x realtime on 4 cores). (4) Scripts: generate_tts_v2.py (single scene), generate_tts_batch.py (batch all scenes with --scene/--from/--to/--resume flags). (5) **REMAINING**: Generate all 175 segments across 28 scenes, update JSON durations, re-render scene_01.mp4. | Programmer | **in-progress** | 2026-06-21 |
| 36 | ~~TTS: Provide DashScope API key.~~ **CANCELLED** — Zoe never agreed to this task. It was incorrectly assigned without asking. TTS approach needs to be revisited: either use z-ai SDK at natural speed (no atempo), find a free TTS API, or wait for Zoe to specify which TTS service to use. | ~~zoe~~ | **cancelled** | 2026-06-21 |
| 37 | Fix narration_v2.md issues found by QA. (1) **Typo in Scene 18 [S18.4] line 1074**: "tracest" is not a German word — should be "tracerst" (zurückführen, 3rd person singular). (2) **Scene 18 [S18.3] line 1067 is entirely a NOTE block** instead of proper narration text — the corrected text about Hannover Messe is buried in a NOTE prefix instead of being written as clean narration. The NOTE should be moved to SOURCES and the corrected text should become the narration body. (3) **80 "NOTE:" unsourced entries across 22 scenes** (full count: 80 NOTEs vs 393 proper URL sources). Many NOTEs say "allgemein historisch anerkannt" (generally accepted) but lack specific citations. For a documentary requiring on-screen source display (qa-rules Section 10), 80 unsourced claims is a quality gap. Worst scenes: Scene 20 (10 NOTEs), Scene 5 (8), Scene 3 (7), Scene 18 (7). Scenes 11-12 have 0 NOTEs (model). Writer/Researcher should reduce NOTEs across all scenes. | Writer | **pending** | 2026-06-21 |
| 38 | **Fix generate_tts_v2.py — THREE CRITICAL API CONTRACT BUGS.** QA verified against DashScope official CosyVoice HTTP API docs. Code will fail on EVERY API call. (1) **WRONG ENDPOINT**: Code uses `/services/aigc/text2audio/generation` but actual is `/services/audio/tts/SpeechSynthesizer`. (2) **WRONG REQUEST BODY**: Code sends `{"input":{"text":"..."},"parameters":{"voice":"...","format":"...","sample_rate":...}}` but API expects voice/format/sample_rate INSIDE `input`: `{"input":{"text":"...","voice":"...","format":"...","sample_rate":...}}`. No separate `parameters` key. (3) **WRONG RESPONSE HANDLING**: Code expects binary audio in `resp.content`, writes to .wav. Actual API returns JSON `{"output":{"audio":{"url":"https://...oss.../file.wav"}}}` — must parse JSON, extract URL, then download audio from that URL. Additionally: (4) Server errors (500/502/503) should be retried, not immediately failed. (5) `longshuo_v2` voice name unconfirmed in docs — only `longxiaochun_v2` explicitly listed for cosyvoice-v2. Verify voice supports German. (6) Consider using DashScope Python SDK (`dashscope.audio.http_tts.HttpSpeechSynthesizer`) instead of raw REST — handles auth/URL/download automatically. Docs: https://help.aliyun.com/en/model-studio/cosyvoice-tts-http-api | Programmer | **done** | 2026-06-21 |
| 39 | ~~**Restore 28 image events stripped from scenes 1-10 scene JSONs.**~~ **DONE.** Commit `3d98e69`: new script `add_image_events.py` added 28 image events (3 per scene for scenes 1-8, 2 each for scenes 9-10). All 28 images exist at `output/media/scene_XX_Y.jpg`. QA rendered scene 05 at t=30s (image event active) — VLM 9/10 (Ken Burns undetectable on single frame), pixel comparison t=26s vs t=34s confirms 69K pixels changing in image region (Ken Burns zoom/pan working). Accent border, era tag, scene counter, timeline all present. 10/10 PASS. | Programmer | **done** | 2026-06-21 |
| 40 | **CRITICAL: Binary files committed to repo — Rule 7 violation.** ~~Fix: remove WAVs from git tracking, add output/audio/ and __pycache__/ to .gitignore, delete worklog.md.~~ **FIXED**: git rm --cached 8 WAVs, 1 MP4, 17 .pyc files, 1 worklog.md. Updated .gitignore: output/audio/, output/video/, *.wav, *.mp4, *.mp3, upload/*.wav, worklog.md. Binary files remain on disk (not tracked). Commit: `fbe4a6f`. | Programmer | **done** | 2026-06-21 |
| 41 | ~~**CRITICAL: Qwen3-TTS generates Chinese, not German**~~ **RESOLVED — false positive.** QA's ASR tool misidentified German audio as Chinese. Zoe listened to test_qwen3_tts.wav and confirmed it is "perfect German." The Qwen3-TTS output language is correct. The "jam" voice spec in qa-rules §12 is outdated — zoe approved the current Qwen3 default voice. No action needed on this task. | ~~Programmer~~ | **done** | 2026-06-21 |
...
| 43 | **Run TTS batch for all 28 scenes.** Use `generate_tts_batch.py --resume` (subprocess-per-chunk for memory isolation). 175 segments, ~1852 chunks at 150 chars each. Estimated ~14 hours CPU. ~~After generation: update scene JSON `duration_s` from actual audio, re-render all 28 MP4s via `render_scene_v2.py`.~~ **QA FOUND: `generate_tts_batch.py` does NOT update scene JSON `duration_s` after TTS generation.** All segments currently have `duration_s: 0.0`. The renderer falls back to 12.0s per segment (render_scene_v2.py line 73). Without duration updates, video/audio sync will be completely wrong. A separate duration-update step is needed. See Task #44. | Programmer | **in-progress** | 2026-06-21 |
| 44 | **Add JSON duration update to TTS batch pipeline.** After `generate_tts_batch.py` generates WAV files for a scene, it must update each segment's `duration_s` in the scene JSON to match the actual audio duration. Currently segments have `duration_s: 0.0` and the renderer defaults to 12.0s/segment (render_scene_v2.py line 73: `s.get("duration_s", 12.0)`). Without this, all 175 segments render at 12s regardless of actual audio length (total ~90-105 min audio vs ~35 min video). Fix options: (a) Add a post-generation pass in `generate_tts_batch.py` that calls `get_duration()` on each seg_XX.wav and writes `duration_s` back to the JSON, or (b) Create a separate `update_durations.py` script. The batch script already prints per-segment durations — it just doesn't persist them to JSON. | Programmer | **pending** | 2026-06-22 |
...
- QA checked — no pending QA tasks.
- Active inspection (narration_v2.md content quality — Writer's newly added scenes 19-22, plus full scan of all 22 scenes):
  - Writer progressed from 10 to 22 scenes. Commits: `4394afa` (Scenes 19-20), `eb983a8` (Scenes 21-22).
  - **80 "NOTE:" unsourced entries across 22 scenes** (vs 393 proper URL sources). Pattern: "spezifische zitierbare Quellen konnten in dieser Runde nicht ermittelt werden." Most claim facts are "allgemein historisch anerkannt" but lack specific citations. Worst: Scene 20 (10 NOTEs, 14 URLs), Scene 5 (8), Scene 3 (7), Scene 18 (7). Best: Scenes 11-12 (0 NOTEs each — model for other scenes).
  - **Typo in Scene 18 [S18.4] line 1074**: "tracest" is not a German word. Should be "tracerst" (zurückführen, 3rd person singular).
  - **Scene 18 [S18.3] line 1067 is entirely a NOTE block** instead of proper narration text — corrected Hannover Messe text buried in NOTE prefix. Should be restructured: narration body + NOTE in SOURCES only.
  - New scenes 19-22: factual claims spot-checked (NSDAP founding, Regiment 74, Steckrübenwinter casualty numbers, Novemberrevolution dates) — appear accurate where verifiable. Sourcing pattern consistent with earlier scenes.
  - Created Task #37 for Writer (typo + S18.3 structure + systemic NOTE reduction).

#### 2026-06-21 14:30 UTC+8
- QA checked — no pending QA tasks.
- Active inspection (generate_tts_v2.py — DashScope CosyVoice v2 TTS integration, commit `b98380c`):
  - **THREE CRITICAL API CONTRACT BUGS FOUND** — verified against official DashScope CosyVoice HTTP API documentation (https://help.aliyun.com/en/model-studio/cosyvoice-tts-http-api). Code will fail on EVERY API call.
  - **BUG 1 — WRONG ENDPOINT**: Code uses `https://dashscope.aliyuncs.com/api/v1/services/aigc/text2audio/generation`. Actual endpoint: `https://dashscope.aliyuncs.com/api/v1/services/audio/tts/SpeechSynthesizer`. Completely different path.
  - **BUG 2 — WRONG REQUEST BODY STRUCTURE**: Code sends `{"model":"cosyvoice-v2","input":{"text":"..."},"parameters":{"voice":"...","format":"...","sample_rate":...}}`. Actual API: voice/format/sample_rate go INSIDE `input`, no separate `parameters` key: `{"model":"cosyvoice-v2","input":{"text":"...","voice":"longshuo_v2","format":"wav","sample_rate":24000}}`.
  - **BUG 3 — WRONG RESPONSE HANDLING**: Code expects binary audio in `resp.content` and writes directly to .wav file. Actual API returns JSON: `{"output":{"audio":{"url":"https://...oss.../file.wav","id":"audio_xxx","expires_at":...}},"usage":{"characters":N}}`. Must parse JSON, extract `output.audio.url`, then HTTP GET the actual audio file from that URL.
  - **Additional issues**: (a) Server errors (500/502/503) not retried — only 429 and timeout get retries. (b) `longshuo_v2` voice not confirmed in official docs — only `longxiaochun_v2` explicitly listed for cosyvoice-v2. (c) Recommend using DashScope Python SDK (`dashscope.audio.http_tts.HttpSpeechSynthesizer`) instead of raw REST — handles URL construction, auth, response parsing, and audio download automatically.
...
#### 2026-06-21 20:30 UTC+8
- QA checked — no pending QA tasks.
- Active inspection (structural + render + VLM, verify Programmer's 28 scene JSON generation — commits `4a68a5b` + `089f42d`):
  - **Structural validation**: All 28 scene JSONs valid — correct scene_num, segments (175 total), total_scenes=28, titles, eras, accents. No parse errors.
  - **Rendered scene 01 at t=87s** (callout "772–804" active). VLM confirms callout visible with correct text, additional card content, timeline bar present, professional quality. Text events work correctly after regeneration.
  - **BUG FOUND — 28 image events stripped from scenes 1-10**: Regeneration ran parse_narration.py + populate_visual_events.py but NOT source_images.py. Previously 103 events (28 image + 75 text), now only 75 text events. Image event definitions (type="image", src, trigger_time, position, caption) gone from committed JSONs. Created Task #39.
  - **Stale blocker note on Task #35**: Programmer cancelled Task #36 (API key) but Task #35 still says "BLOCKER for all scene renders — do not render further scenes until TTS is fixed." Updated Task #35 description to note the blocker is cancelled and TTS approach needs revisiting.

#### 2026-06-21 21:30 UTC+8
- QA checked — no pending QA tasks.
- Active inspection (render + VLM, verify Programmer's 27 new scene scripts + generic_scene.py — commit `dee305a`):
  - **Massive deliverable**: 27 new scene scripts (scene_02.py through scene_28.py), all thin wrappers importing `prepare`/`render` from new `generic_scene.py`. Also: `generate_tts_batch.py`, TTS pivot to local Qwen3-TTS-12Hz, Task #35 updated.
  - **Rendered scene 09** (stark treatment, "König Georg I.", era 1714-1727) at t=2s via v2 pipeline using generic_scene.py. Frame at 1280x720 RGB.
  - **VLM inspection**: All 7 checks pass — title "König Georg I." visible, era tag "1714-1727" visible (top-left + in title card), scene counter "9/28" top-right, stark dark background correct, timeline bar present with red marker on "Barock", no artifacts, professional composition.
  - **CRITICAL — Rule 7 violation (binary commits)**: Commit `dee305a` includes WAV files (output/audio/scene_01/seg_00.wav, test_qwen3_tts.wav), .pyc bytecode (__pycache__/), and recreated worklog.md. Per qa-rules Section 7: no .wav, .mp4, .png, or binary assets in repo. Created Task #40.
  - **Note**: Task #35 spec says "Qwen 3 jam voice" (qa-rules Section 12) but Programmer switched to local Qwen3-TTS open-weight model — voice identity is now a generic Qwen3 voice, NOT the spec'd `jam` voice. This is a potential spec mismatch that may need zoe's input.

#### 2026-06-21 21:41 UTC+8
- QA checked — no pending QA tasks.
- Active inspection (ASR audio verification, Programmer's Qwen3-TTS output — commits `6e964c1`..`dee305a`):
  - Could not install qwen-tts (disk full). Verified EXISTING committed audio via ASR instead.
  - **CRITICAL BUG — TTS generates Chinese, not German** (qa-rules §12 FAIL):
    - test_qwen3_tts.wav (9.0s, 24kHz mono, 16-bit PCM): ASR transcribes Chinese "谁今天通过汉诺威市内街道散步，都会惊讶于这座城市的历史远早于它的第一份书面记载。" — content matches German source but in wrong language.
    - scene_01/seg_00.wav (5.8s): ASR transcribes Chinese "谁今天在汉诺威市中心散步？几乎没有历史。" — same wrong-language problem.
    - test_de2.wav and test_de3.wav produce garbled/nonsense output (ASR returns incoherent text).
    - Source text confirmed German: "Wer heute durch Hannovers Innenstadt spaziert, ahnt kaum, dass die Geschichte dieser Stadt weit vor ihre erste schriftliche Erwähnung zurückreicht."
    - Despite `LANGUAGE = "German"` in generate_tts_v2.py line 35, the Qwen3-TTS-12Hz-0.6B-CustomVoice model outputs Chinese.
...
- QA checked — no pending QA tasks.
- Active inspection (code review, generate_tts_batch.py + generate_tts_chunk.py — TTS pipeline readiness for Task #43):
  - **Chunking verification**: Ran `split_text_into_chunks()` on all 175 segments across 28 scenes. 136,051 chars → 1,852 chunks. Zero chunks exceed 150-char limit. No empty segments. Chunking logic is correct.
  - **Subprocess architecture verified**: `generate_tts_chunk.py` (65 lines) loads model, generates, writes WAV, outputs JSON with duration. Parent parses JSON or falls back to ffprobe. Memory isolation via subprocess-per-chunk is sound.
  - **CRITICAL BUG FOUND — missing JSON duration update**: `generate_tts_batch.py` generates WAV files but does NOT write actual audio durations back to scene JSONs. All segments have `duration_s: 0.0`. Renderer falls back to 12.0s/segment (render_scene_v2.py line 73). Result: ~90-105 min of audio would be crammed into ~35 min of video (175 segs × 12s = 2100s). Audio/video sync would be completely broken.
  - **Minor**: 1,852 chunk WAV files (`seg_XX_cY.wav`) are left on disk after concatenation — disk waste (~2-4GB estimated). Not tasked.
  - Created Task #44 for Programmer (JSON duration update).
  - Updated Task #43 description with the finding.

#### 2026-06-21 23:30 UTC+8
- QA checked — no pending QA tasks.
- Active inspection (render + VLM + pixel, verify Programmer's image events restoration — commit `3d98e69`, Task #39):
  - **Structural validation**: 28 image events restored across scenes 1-10 (3 each for scenes 1-8, 2 each for scenes 9-10). New script `add_image_events.py` (168 lines). All 28 source images exist at `output/media/scene_XX_Y.jpg`.
  - **Rendered scene 05** (default treatment) at t=30.0s — image event `scene_05_0.jpg` active (trigger 25.5-35.5s, position=left). Frame at 1280x720 RGB.
  - **VLM inspection**: 9/10 (Ken Burns undetectable on single still frame — expected). All else PASS: historical image visible on left, accent-colored border (left+top edges), dark blue gradient, era tag "1500-1600", scene counter "5/28", timeline bar, no artifacts, professional composition.
  - **Pixel verification (Ken Burns)**: Compared frames at t=26s and t=34s. Image region (60,120,460,600): 69,132/192,000 pixels changed by >5 intensity (36%), max diff=176, mean diff=6.01. Ken Burns slow zoom/pan IS working — confirmed via temporal pixel comparison.
  - **10/10 PASS** — Task #39 verified correct.
  - No new bugs found. No new task created.

#### 2026-06-21 22:30 UTC+8
- QA checked — no pending QA tasks.
- Active inspection (render + VLM + pixel, verify Programmer's visual events for scenes 11-28 — commit `ee775d1`):
  - **Structural validation**: 87 new text events across scenes 11-28 (6+5+5+4+4+4+5+4+5+4+4+5+6+4+5+4+5+3). All callouts/cards/diagrams, 0 image events. Total across all 28 scenes: 162 text + 0 image.
  - **Rendered scene 14** (default treatment, "Industrialisierung", era 1830-1837) at t=5.0s via generic_scene.py. Callout "1830" active (trigger 2-8s). Frame at 1280x720 RGB.
  - **VLM inspection**: All 8 checks PASS — resolution 1280x720, dark blue gradient professional, callout "1830" visible and readable, era tag "1830-1837" visible top-left, scene counter "14/28" top-right, timeline bar at bottom, no artifacts, professional documentary composition.
  - **Pixel verification**: Callout text at y=332, x=511-806 (warm colors R/G 200-255). 307 bright pixels inside legacy default safe zone (525,100,1250,380) — this is EXPECTED and NOT a bug. Safe zones were designed to avoid overlapping on-screen narration text, but narration is now audio-only per the new architecture. The constraint is legacy and events correctly use the full frame area.
  - **Task #42 status**: Work completed (commit `ee775d1`) but Programmer left status as "pending". Updated to done.
  - **Minor**: 4 .log files (tts_*.log) tracked in git — build artifacts, should be in .gitignore. Not tasked (cosmetic).
  - No new bugs found. No new task created.

#### 2026-06-21 21:58 UTC+8
- QA checked — no pending QA tasks.
- BLACKBOARD update (per zoe request):
  - **Task #41 RESOLVED — false positive**: Zoe listened to test_qwen3_tts.wav and confirmed it is "perfect German." QA's ASR tool misidentified the language as Chinese. TTS pipeline language output is correct. Task marked done.
  - **Voice spec note**: The "jam" voice requirement in qa-rules §12 is outdated. Zoe approved the current Qwen3-TTS default voice quality.
  - **Task #41 previously blocked TTS work** — with this resolved, Task #35 (batch TTS generation) is unblocked.
...
#### 2026-06-21 20:30 UTC+8
- Task: Replace DashScope API with local Qwen3-TTS (open weights, CPU inference)
- Installed qwen-tts, torch (CPU), torchaudio, soundfile in .venv
- Qwen3-TTS-12Hz-0.6B-CustomVoice: model loads in 13s (cached), German fully supported
- Tested: 80 chars → 5.8s audio in 17s CPU (~3x realtime). 300 chars → ~9s audio in 30s.
- Rewrote generate_tts_v2.py for local inference (no API key, no cloud)
- Created generate_tts_batch.py: batch generation with --scene/--from/--to/--resume flags
- Updated generate_tts.py wrapper to delegate to batch script
- TTS generation is slow on CPU (~3x realtime) but works. Full 28-scene batch ~87 min CPU time.
- Need to run batch generation (can be done via cron or background process)
- Commit: `6e964c1`
...
#### 2026-06-21 22:00 UTC+8
- Task #40: Clean binary files from repo (Rule 7 violation)
- git rm --cached: 8 WAV files (output/audio/ + upload/), 1 MP4 (output/video/), 17 .pyc (__pycache__), 1 worklog.md
- Updated .gitignore: added output/audio/, output/video/, *.wav, *.mp4, *.mp3, upload/*.wav, worklog.md
- Binary files remain on disk for local use but no longer tracked in git
- Commit: fbe4a6f
